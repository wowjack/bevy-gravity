...
use crate::{physics::{ChangeEvent, MassiveObject}, visual_object::VisualObjectBundle};
    ...
    SidePanel::new(panel::Side::Right, "sidepanel")
        ...
        .show(contexts.ctx_mut(), |ui| {
            ...
            if ui.button("spawn").clicked() {
                let entity = commands.spawn(VisualObjectBundle::default()).id();
                change_event_writer.send(ChangeEvent { entity, change: crate::physics::Change::CreateObject(
                    MassiveObject { position: DVec2::ZERO, velocity: DVec2::ONE, mass: 1. }
                ) });
            }
        });
}